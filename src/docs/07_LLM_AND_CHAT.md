...
The SDK exposes chat functionality through `sogni.projects.chatCompletion()`.
...
// Round 1
const turn1 = await sogni.projects.chatCompletion({ messages, ... });
messages.push(turn1.choices[0].message);

// Round 2
messages.push({ role: 'user', content: 'Who was the MVP?' });
const turn2 = await sogni.projects.chatCompletion({ messages, ... });
```
